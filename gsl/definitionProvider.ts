        ...
        if (txt.includes("call")) {
            ...
            if (txtArray[4] === "$thisscript") {
                for (let i = 0; i < document.lineCount; i++) {
                    ...
                    if (
                        line.text.toLowerCase().startsWith(": " + txtArray[2])
                    ) {
                ...
                if (txtArray.length === 2) {
                    // call #
                    script = txtArray[1];
                } else if (txtArray[3] === "in") {
                    // callmatch must_match "$*" in #
                    script = txtArray[4];
                } else {
                    return;
                }
                ...
                if (!fs.existsSync(scriptFile)) {
                    ...
                    if (!this.enableAutomaticDownloads) {
                        console.warn(
                            "File not found and automatic downloads disabled",
                        );
                        return;
                    }
                    ...
                    if (inFlight) {
                        ...
                        try {
                            ...
                            await vsc!.withEditorClient((client) => {
                                return GSLExtension.downloadScript(
                                    client,
                                    Number(script),
                                );
                            });
