...
const PUBKEY =
    "";
...
export interface GameOptionCollection {
    [index: string]: GameOption;
}
export interface CharacterOptionCollection {
    [index: string]: CharacterOption;
}
...
export class EAccessClient extends EventEmitter {
    ...
    connect() {
        if (this.status === STATUS_NEW) {
            this.socket = tls_connect(
                ...
                () => {
                    const certificate =
                        this.socket.getPeerCertificate() as RSAPeerCertificate;

                    if (
                        certificate.pubkey.toString("hex").toUpperCase() !==
                        PUBKEY.toUpperCase()
                    ) {
                        this.console.log(
                            `Certificate pubkey ${certificate.pubkey.toString("hex")} does not match expeted value.`,
                        );
                        this.socket.end();
                        this.emit(
                            EVENT_ERROR,
                            new Error(
                                "Could not establish secure connection for authentication; bad certificate.",
                            ),
                        );
                        return;
                    }
                    this.console.log(
                        "EAccess::socket::secureConnect",
            ...
            this.socket.on("error", (err) => {
                if (this.debug) {
                    this.console.log("EAccess:socket:error", err);
                }
            ...
            this.socket.on("end", () => {
                if (this.debug) {
                    this.console.log("EAccess:socket:end");
                }
                this.emit(EVENT_END);
                this.close();
            });
            this.socket.on("close", (errored) => {
                if (this.debug) {
                    this.console.log(
                        "EAccess:socket:close",
                        errored ? "(with error)" : "(ok)",
                    );
                }
            ...
            if (this.debug) {
                this.console.log(
                    "EAccess#tls_connect : connecting to",
                    this.host,
                    "on port",
                    this.port,
                );
            ...
            this.emit(
                EVENT_ERROR,
                new Error("EAccessClient status must be new to connect."),
            );
        }
    }
    ...
    authorize(account: string, password: string) {
        if (this.status == STATUS_READY) {
            ...
            this.emit(
                EVENT_ERROR,
                new Error("EAccessClient status must be ready to authorize."),
            );
        }
    }
    ...
    private hashPassword(password: string): Buffer {
        let n,
            nn,
            pw = Buffer.from(password),
            hash = Buffer.alloc(pw.length);
        if (this.hash === null) {
            return hash;
        }
    ...
    private a11n_response(data: Buffer) {
        ...
        if (this.debug) {
            this.console.log("EAccess: A =>", JSON.stringify(reply));
        }
        switch (result) {
            case "KEY":
                break;
            ...
            case ERROR_PASSWORD:
                if (this.debug) {
                    this.console.log("EAccess Error", result, response);
                }
            ...
            default:
                if (this.debug) {
                    this.console.log(
                        "EAccess unhandled A response: %s",
                        response,
                    );
                }
    ...
    private receiveGameList(data: Buffer) {
        ...
        let reply = response.split(RX_TAB_NL);
        if (this.debug) {
            this.console.log("EAccess: M =>", JSON.stringify(reply));
        }
        let codes = [];
    ...
    private checkGameAccess(codes: Array<string>) {
        ...
        this.socket.once("data", (data) => {
            ...
            if (this.debug) {
                this.console.log("EAccess: F =>", JSON.stringify(reply));
            }
            let game = this.games[code];
            if (game) {
                game.access = access.toLowerCase();
            }
            if (codes.length > 0) {
                this.checkGameAccess(codes);
            } else {
                this.emit(EVENT_GAMES, this.games, this.gameCodes);
            }
        });
    ...
    private selectGame_response(data: Buffer) {
        ...
        if (this.debug) {
            this.console.log("EAccess: G =>", JSON.stringify(reply));
        }
        if (code !== "G") {
    ...
    private selectGame_N_response(data: Buffer) {
        ...
        let [code, type] = reply;
        if (this.debug) {
            this.console.log("EAccess: N =>", JSON.stringify(reply));
        }
        if (code !== "N") {
            this.emit(EVENT_PROBLEM, code, response);
            this.close();
    ...
    private selectGame_C_response(data: Buffer) {
        ...
        if (this.debug) {
            this.console.log("EAccess: C =>", JSON.stringify(reply));
        }
        if (code !== "C") {
            this.emit(EVENT_PROBLEM, code, response);
            this.close();
    ...
    selectCharacter(code: string, mode: string) {
        if (!(mode == "play" || mode == "storm")) {
            mode = "play";
        }
        if (this.selectedGame.storm) {
            mode = "storm";
        }
        this.selectedCharacterCode = code;
        this.socket.once("data", (data) =>
            this.selectedCharacter_L_response(data),
        );
    ...
    private selectedCharacter_L_response(data: Buffer) {
        ...
        if (this.debug) {
            this.console.log("EAccess: L =>", JSON.stringify(reply));
        }
        if (code !== "L" || status !== "OK") {
            this.emit(EVENT_PROBLEM, code, response);
            this.close();
    ...
    static login(
        ...
        password: string,
        filter?: GameFilter,
    ): Promise<GameOptionChoice> {
        function applyFilter(
            games: GameOptionCollection,
            filter: GameFilter,
        ...
        return new Promise((resolve, reject) => {
            ...
            client.once(
                EVENT_GAMES,
                (games: GameOptionCollection, codes: Array<string>) => {
...
class GameOptionChoice {
    ...
    pick(name: string): string {
        return (
            this.codes.find((code) => this.games[code].name === name) ||
            "unknown"
        );
    }
...
class CharacterOptionChoice {
    ...
    pick(name: string): string {
        return (
            this.codes.find((code) => this.characters[code].name === name) ||
            "unknown"
        );
    }
    select(
        ...
        return new Promise((resolve, reject) => {
            ...
            this.client.once(
                EVENT_LAUNCH,
                (sal: SAL, loginDetails: LoginDetails) => {
                    this.client.removeAllListeners(EVENT_PROBLEM);
                    this.client.removeAllListeners(EVENT_CLOSE);
...
class EAccessError extends Error {
    ...
    constructor(code: string) {
        ...
        const message: string =
            errorCodes[code] ||
            `An unknown error code ${code} has been encountered.`;
        super(message);
        this.code = code;
    }
}
