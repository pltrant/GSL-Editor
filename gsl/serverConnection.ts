...
export class ServerConnection extends EventEmitter {
    ...
    connect() {
        ...
        if (this.key === null) {
            throw new Error("Cannot connect with null key.");
        }
    ...
    send(data: string) {
        if (this.debug === true) {
            this.console.log("Connection#send", JSON.stringify(data));
        }
    ...
    close(quit?: string) {
        if (this.state === ServerConnectionState.Closed) {
            return;
        }
        ...
        if (this.debug === true) {
            this.console.log("Connection#close");
        }
        this.socketEnd();
    }
    ...
    private processBuffer(newLine: string) {
        let eol = this.receiveBuffer.indexOf(newLine, 0),
            last = newLine.length * -1;
        while (eol > -1) {
    ...
    private setConnectionMode(mode: ServerConnectionMode) {
        if (this.debug === true) {
            this.console.log("Connection#setConnectionMode()", mode.toString());
        }
        this.emit("mode", mode);
    ...
    private socketCleanup() {
        if (this.debug === true) {
            this.console.log("Connection#socketCleanup()");
        }
    ...
    private socketError(error: Error) {
        if (this.debug === true) {
            this.console.log("Connection#socketError()", error);
        }
        this.socketCleanup();
        this.socket.destroy();
    ...
    private socketClose() {
        if (this.debug === true) {
            this.console.log("Connection#socketClose()");
        }
        this.socketCleanup();
    ...
    private socketData(data: Buffer) {
        ...
        if (this.debug === true) {
            this.console.log("Connection#socketData", JSON.stringify(text));
        }
    ...
    private socketEnd() {
        if (this.debug === true) {
            this.console.log("Connection#socketEnd()");
        }
        if (this.socket.writable === true) {
            this.socket.end();
        }
