...
export class OutOfDateButtonManager {
    private OUT_OF_DATE_BUTTON_CLICKED_CMD =
        "internal.outOfDateManager.outOfDateClicked";
    private IGNORED_SCRIPT_BUTTON_CLICKED_CMD =
        "internal.outOfDateManager.ignoredClicked";
    ...
    public renderButton(state: OutOfDateButtonState): void {
        ...
        switch (state.state) {
            ...
            case "out_of_date": {
                ...
                this.button.backgroundColor = new ThemeColor(
                    "statusBarItem.warningBackground",
                );
                this.button.color = new ThemeColor(
    ...
    private async runCheck() {
        ...
        await this.withEditorClient(async (client) => {
            ...
            LAST_CHECK_MAP.set(document.uri.toString(), {
        ...
        const pollingDelay =
            DELAY_MAP[
                workspace
                    ...
                    .get("scriptPollingDelay") as keyof typeof DELAY_MAP
            ] || 0;
    ...
    private async calculateState(
        ...
        // Compare time stamps
        const lastSeenMod =
            GSLExtension.findLastSeenScriptModification(scriptNum);
        ...
        if (
            this.isIgnoringScript(scriptNum, scriptProperties.lastModifiedDate)
        ) {
    ...
    private isIgnoringScript(
        scriptNum: number,
        serverLastModifiedDate: Date,
    ): boolean {
        return (
            this.context.globalState.get<{ lastModifiedDate: string }>(
                this.getIgnoreKey(scriptNum),
            )?.lastModifiedDate === serverLastModifiedDate.toISOString()
        );
    }

    private ignoreScript(
        scriptNum: number,
        serverLastModifiedDate: Date,
    ): void {
        this.context.globalState.update(this.getIgnoreKey(scriptNum), {
            lastModifiedDate: serverLastModifiedDate.toISOString(),
        });
    }
    ...
    private async showDiff(
        ...
        try {
            ...
            // Open diff view
            await window.showTextDocument(Uri.file(oldFilePath), {
    ...
    private async stopCheckingScript(
        ...
        const lastModifiedDate =
            (await GSLExtension.checkModifiedDate(client, scriptNum)) ||
            GSLExtension.findLastSeenScriptModification(scriptNum)
                ?.lastModifiedDate;
        if (!lastModifiedDate) {
