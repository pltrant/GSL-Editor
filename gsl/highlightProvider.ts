...
export class GSLDocumentHighlightProvider implements DocumentHighlightProvider {
        ...
        if (
            ...
            if (blockEnd) {
                ...
                if (startOfBlock) {
                    ...
                    return this.provideDocumentHighlights(
                        document,
                        startOfBlock,
                    );
                }
            ...
            if (blockStart) {
                return [
                    ...
                    ...this.scanForwardsHighlightBlockTokens(
                        document,
                        position,
                        blockStart,
                    ),
    ...
    private translateWordToRegex(
        ...
        if (nodeMatch) {
            ...
            return new RegExp(
                `\\b${word}\\b|\\$${tokenPrefix}${num}[a-z]?`,
                "gi",
            );
        }
        // Handle tokens by matching on range with leading dollar sign
        const rangeWithDollar = document.getWordRangeAtPosition(
            position,
            /\$\w+\b/,
        );
        const wordWithDollar = rangeWithDollar
            ? document.getText(
                  document.getWordRangeAtPosition(position, /\$\w+\b/),
              )
            : undefined;
        ...
        if (tokenMatch) {
            ...
            return new RegExp(
                `\\bN${nodePrefix}${num}\\b|\\$${tokenPrefix}${num}[a-z]?`,
                "gi",
            );
        }
        // Handle normal words
    ...
    private highlightEachInstanceOfWord(
        ...
        for (let lineNum = 0; lineNum < document.lineCount; lineNum++) {
            ...
            while ((match = regex.exec(lineText)) !== null) {
                ...
                highlights.push(
                    new DocumentHighlight(range, DocumentHighlightKind.Text),
                );
            }
        }
    ...
    private scanForwardsHighlightBlockTokens(
        ...
        for (
            ...
            if (originalMatch === "when" && blockStack.length === 2) {
                ...
                if (endCaseStatement) {
                    highlights.push(
                        highlightMatch(document, i, endCaseStatement),
                    );
                }
            }
        ...
        for (
            ...
            if (blockStart) {
                ...
                if (!blockStack.length) {
                    return new Position(
                        i,
                        line.text.toLowerCase().indexOf(blockStart),
                    );
                }
            }
    ...
    private scanBackwardHighlightPushes(
        ...
        for (
            ...
            const blockEnd =
                readToken(line.text, POP) ?? readToken(line.text, DOT);
            if (blockEnd) {
                if (
                    (blockEnd === "pop" || blockEnd === "fastpop") &&
                    earlyStop
                ) {
...
const peekStack = <T>(stack: T[]): T | null =>
...
const isPair = (blockStartMatch: string, blockEndMatch: string): boolean => {
