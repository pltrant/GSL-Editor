...
export const MESSAGE_LINE_REGEX =
    /^(\s*)(msgp|msg\s+NP\d|msgr|msgrxp|msgrx2)\s+"(.*?)(\$\\)?"\s*(!\s*.*)?/i;
const WRAPPABLE_LINE_REGEX =
    /^(\s*)(msgp|msg\s+NP\d|msgr|msgrxp|msgrx2|set.*?to)\s*\(?"(.*)"\s*(!.*)?/i;
...
export const isWrappedString = (text: string): boolean =>
    Boolean(
        text.trimStart()[0] !== "!" &&
...
export const getMessageCommand = (text: string): MessageCommand | null => {
    ...
    if (match) {
        const [_, indentation, command, content, skipNewlineSymbol, comment] =
            match;
        return {
            indentation,
            command,
...
const BLOCK_START_REGEX =
    /^(if|ifnot|else_if|else_ifnot|else|loop|when|push|fastpush|push|is|default|:)/i;
