...
type GetScriptProperties = (
    script: number,
) => Promise<ScriptProperties | undefined>;
...
export class GSLHoverProvider implements HoverProvider {
    ...
    constructor(
        ...
        this.baseHoverRegex =
            /^\s*[a-zA-Z]+( |$)|\$(:\$[A-Z]+|:\d+\[\d+,\d+,\d+\]|[\w\d:_-]+|[ABDVLSKT]\d|[$\\^QR*+'])/;
    ...
    provideHover(
        ...
        position: Position,
        token: any,
    ): Hover | Promise<Hover | undefined> | undefined {
        // Check for hover over callmatch script number
        if (
            document.getWordRangeAtPosition(
                position,
                this.callmatchScriptNumberRegex,
            )
        ) {
    ...
    stringTokenHover(token: any): any {
        ...
        if (
            tokenTypes &&
            tokenTypes[1] in this.nodeInfo &&
            tokenTypes[3] in this.nodeInfo[tokenTypes[1]]
        ) {
            return new Hover(
                "N" +
                    tokenTypes[1].toUpperCase() +
                    tokenTypes[2] +
                    ": " +
                    this.nodeInfo[tokenTypes[1]][tokenTypes[3]],
            );
        }
    }
    ...
    varHover(token: any): any {
        ...
        return new Hover(
            varName + tokenTypes[2] + ": " + this.varInfo[tokenTypes[1]],
        );
    }
    ...
    tableHover(word: any): any {
        ...
        return new Hover(
            "table #" + tokenTypes[1] + ": value in " + tokenTypes[2],
        );
    }

    async callmatchHover(
        script: number,
        document: TextDocument,
        ...
        if (scriptProperties) {
            const { desc, name, owner, modifier, lastModifiedDate } =
                scriptProperties;
