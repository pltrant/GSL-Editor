...
suite("GSLCodeActionProvider Test Suite", () => {
    ...
    async function applyCodeAction(
        ...
        const actions = await Promise.resolve(
            provider.provideCodeActions(document, range, context),
        );
        ...
        assert.ok(
            ...
            "Expected at least one TextEdit in WorkspaceEdit",
        );
    ...
    test("should handle a long unbroken string", async () => {
        const longText =
            '  msgp ("testingtestingtestingtestingtestingtestingtestingtestingtestingtestingtestingtestingtestingtestintestingtestingtestingtestingtestingtestingtestingtestingtestingtestingtestingtestingtestingtestinFoobarfoo")';
        const replacement = await applyCodeAction(
            longText,
            ACTION_REDISTRIBUTE_LINES,
        );
    ...
    test("should collapse multiline string", async () => {
        const multiLineString = `msgp ("This is a test" +\\
            " that spans multiple" +\\
            " lines")`;
        assert.equal(
    ...
    test("should combine messages if the line skips newline and the command is identical", async () => {
        const multiLineString = `    msgp "Foo$\\"\n` + `    msgp "Bar"`;
        assert.equal(
            await applyCodeAction(multiLineString, ACTION_REDISTRIBUTE_LINES),
            `    msgp "FooBar"`,
        );
